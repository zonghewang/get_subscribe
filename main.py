...
def get_open_runner_url():
    ...
    for i in [0,1,2,3,4]:
      urlv2 = 'https://clashnode.com/wp-content/uploads/'+dt.strftime('%Y')+'/'+dt.strftime('%m')+'/'+str(i)+'-'+dt.strftime('%Y%m%d')+'.txt'
      urlclash = 'https://clashnode.com/wp-content/uploads/'+dt.strftime('%Y')+'/'+dt.strftime('%m')+'/'+str(i)+'-'+dt.strftime('%Y%m%d')+'.yaml'
      print(urlv2)
      print(urlclash)
      v2 = requests.get(urlv2)
      clash = requests.get(urlclash)
      with open(f'./subscribe/openRunner/v2ray-{i}.txt','wb') as v2f:
        v2f.write(v2.content)
        v2f.close
    
      with open(f'./subscribe/openRunner/clash-{i}.yml','wb') as clashf:
        clashf.write(clash.content)
        clashf.close
        gbk_trans_utf8(f'./subscribe/openRunner/clash-{i}.yml')
        replase_4_reg(f'./subscribe/openRunner/clash-{i}.yml')
