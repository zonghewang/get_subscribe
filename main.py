...
def get_open_runner_url():
    ...
    urlv2 = 'https://clashnode.com/wp-content/uploads/'+dt.strftime('%Y')+'/'+dt.strftime('%m')+'/'+dt.strftime('%Y%m%d')+'.txt'
    urlclash = 'https://clashnode.com/wp-content/uploads/'+dt.strftime('%Y')+'/'+dt.strftime('%m')+'/'+dt.strftime('%Y%m%d')+'.yaml'
    print(urlv2)
    print(urlclash)
    v2 = requests.get(urlv2)
    clash = requests.get(urlclash)
    with open('./subscribe/openRunner/v2ray.txt','wb') as v2f:
      v2f.write(v2.content)
      v2f.close
  
    with open('./subscribe/openRunner/clash.yml','wb') as clashf:
      clashf.write(clash.content)
      clashf.close
      # gbk_trans_utf8('./subscribe/openRunner/clash.yml')
      # replase_4_reg('./subscribe/openRunner/clash.yml')
